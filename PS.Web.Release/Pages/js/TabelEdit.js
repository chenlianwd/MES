...
{
    if (gList[column.listName])
    {
        for (var i = 0; i < gList[column.listName].length; i++)
        {
            if (gList[column.listName][i]['_ID'] == rowdata[column.columnname])
                return gList[column.listName][i]['Name'];
        }
    }
...
function f_Submit(item, grid)
{
    ...
    var Changes = gEnabledSort ? grid.getChanges() : grid.getData(undefined,false,true);
    $.ajax({
        type: 'post',
...
function f_OperateUpDown(rowdata, rowindex, value)
{
    var h = "";
    if (rowdata._editing)
    {
