...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        MarkDivisor: 3 //使用l-grid-row-cell样式的行号除数
    };
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        pageStatMessage: '从{from}到{to}，总 {total} 条 。每页：{pagesize}',
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _setToolbar: function (value)
        {
            ...
            if (value && $.fn.ligerToolBar)
            {
                ...
                g.toolbarManager = g.topbar.ligerToolBar(value, this);
                if (value.title)
                {
        ...
        */
        ...
        getData: function(status, removeStatus, keepIndex)
        {
            var g = this, p = this.options;
            var data = [];
            ...
            for (var rowid in g.records)
            {
                ...
                {
                    data.push(g.formatRecord(o, removeStatus, keepIndex));
                }
            }
            return data;
        },
        //格式化数据
        formatRecord: function(o, removeStatus, keepIndex)
        {
            delete o['__id'];
            delete o['__previd'];
            delete o['__nextid'];
            if (!keepIndex) delete o['__index'];
        ...
        _initBuildHeader: function ()
        {
            ...
            if (p.toolbar)
            {
                if ($.fn.ligerToolBar)
                {
                    g.toolbarManager = g.topbar.ligerToolBar(p.toolbar, this);
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                if (item['__index'] % p.MarkDivisor != p.MarkDivisor - 1 && p.alternatingRow)
