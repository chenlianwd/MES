    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                treehtmlarr.push('<span class="l-content">' + o[p.textFieldName]
                     + '</span></div>');
                if (g.hasChildren(o))
                {
                    var isLastNew = [];
        ...
        _setTreeEven: function ()
        {
            ...
            { 
                var obj = (e.target || e.srcElement);
                var treeitem = null;
                if (obj.tagName.toLowerCase() == "a" || obj.tagName.toLowerCase() == "span" || $(obj).hasClass("l-box") || $(obj).hasClass("l-content"))
                    treeitem = $(obj).parent().parent();            
                else if ($(obj).hasClass("l-body"))
                    treeitem = $(obj).parent();
                else
