...
namespace AutoSolder.DAL
{
    public class AccessDBBase
    {
        ...
        public static bool ExistMysqlDB()
        {
            ...
            try
            {
                string rootmysqlStr = "Data Source=localhost;User Id=root; Password=pempenn;pooling=false;CharSet=utf8;port=" + GlobalData.MySqlPort + ";";
            ...
            catch (MySqlException e)
            {
                //捕获到无法建立连接即未安装数据库
                issuc = false;
                LogClass.WriteLogFile(e.Message);
            }
            return issuc;
        }
        /// <summary>
        ...
        /// <returns></returns>
        public static bool ExistDB(string dbname, string user, string pwd, int port)
        {
            bool issuc = true;
        ...
        public static bool CreateDB(string dbname, string user, string pwd)
        {
            bool issuc = true;
        ...
        /// </summary>
        /// <returns></returns>
        public static MySqlConnection getMySqlCon(string dbname, string user, string pwd, int port)
        {
        ...
        /// <returns></returns>
        public static bool ExistTabal(string dbname, string user, string pwd, int port, string tablename)
        {
            bool issuc = true;
        ...
        /// <returns></returns>
        public static bool ExecuteCommand(string dbname, string user, string pwd, int port, string command)
        {
            bool issuc = true;
        ...
        {
            string rootmysqlStr = "Database=profile; Data Source=localhost;User Id=root; Password=Root123456;pooling=false;CharSet=utf8;port=" + GlobalData.MySqlPort;
            using (MySqlConnection connection = new MySqlConnection(rootmysqlStr))
            {
                ...
                catch (MySqlException ex)
                {
                    ...
                    LogClass.WriteLogFile("批量事务处理出错：" + ex.Message);
                }
                finally
                {
        ...
        public static DataTable GetDataTable(MySqlConnection connection, string sqlCmd_select, params MySqlParameter[] cmdParams)
        {
            ...
            try
            {
                ...
                {
                    Dt = null;
                    LogClass.WriteLogFile("DataTable填充数据异常" + ex.Message);
                }
            }
            catch (MySqlException ex)
            {
        ...
        public static List<T> MakeTableToPackage<T>(T t, MySqlConnection connection, string sqlCmd_select, params MySqlParameter[] cmdParams) where T : class, new()//where T:class,new() 表示类型T必须是类，并且可以实例化
        {
            ...
            {
                LogClass.WriteLogFile("打包数据出错-MakeTablePackage" + ex.Message);
